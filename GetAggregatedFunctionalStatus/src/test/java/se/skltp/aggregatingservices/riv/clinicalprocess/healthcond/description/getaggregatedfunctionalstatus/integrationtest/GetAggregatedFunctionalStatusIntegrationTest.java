...
public class GetAggregatedFunctionalStatusIntegrationTest extends AbstractAggregateIntegrationTest {
    ...
    private static final String DEFAULT_SERVICE_ADDRESS = GetAggregatedFunctionalStatusMuleServer.getAddress("SERVICE_INBOUND_URL");

    protected String getConfigResources() {
        return "soitoolkit-mule-jms-connector-activemq-embedded.xml," + 
               "GetAggregatedFunctionalStatus-common.xml," +
             // Only load GetAggregatedFunctionalStatus-common.xml - it will import the other config files 
    ...
    private List<ProcessingStatusRecordType> doTest(String registeredResidentId, String senderId, String originalConsumerHsaId,
        ...
        GetAggregatedFunctionalStatusTestConsumer consumer = new GetAggregatedFunctionalStatusTestConsumer(DEFAULT_SERVICE_ADDRESS, senderId,
        ...
        for (int i = 0; i < testData.length; i++) {
            ...
            assertNotNull(responseElement.getFunctionalStatusAssessmentHeader());
            assertNotNull(responseElement.getFunctionalStatusAssessmentHeader().getPatientId());
            assertEquals(registeredResidentId, responseElement.getFunctionalStatusAssessmentHeader().getPatientId().getId());
            assertNotNull(responseElement.getFunctionalStatusAssessmentHeader());
            assertEquals(testData[i].getExpectedBusinessObjectId(), responseElement.getFunctionalStatusAssessmentHeader().getDocumentId());
            assertEquals(testData[i].getExpectedLogicalAddress(), responseElement.getFunctionalStatusAssessmentHeader().getSourceSystemHSAId());
        }
